...
def plot_confusion_matrix(normalized_confusion_matrix, y_labels=None,
    ...
    plt.tight_layout()
    plt.savefig(path.joinpath(filename), dpi=300)
    if with_print:
        plt.show()

def combined_perf_eval(y_test, predictions, labels, model_name,
                       path_performance):
    metrics = performance_metrics(y_test, predictions)
    norm_matrix, matrix = confusion_matrix(y_test, predictions, labels)
    plot_confusion_matrix(norm_matrix, labels, path=path_performance.joinpath(model_name),
                          filename=model_name)
