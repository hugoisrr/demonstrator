...
const Sidebar = () => {
  ...
  if (allWks.length > 0) {
    allWks = allWks.map((item, i) => {
      return (({ ws_id, ws_name, source }) => ({ ws_id, ws_name, source }))(
        allWks[i]
      );
    });
  ...
  uniqueWks.forEach(element => {
    uniqueDict.push({key : element, ws_id : element, labeler : false, model : false, device : false})
  });
...
function Display(param){
  console.log(param)
  if (param['param']){
    return  <td><svg height="30" width="30">
    <circle cx="15" cy="15" r="10" stroke="black" stroke-width="2" fill="green" />
  </svg> </td>
  }
  return <td><svg height="30" width="30">
